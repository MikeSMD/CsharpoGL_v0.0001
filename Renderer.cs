							...
							for ( int x = xmin; x < xmax; ++x )
							{
								for ( int y = ymin; y < ymax; ++y )
								{
									...
									/*
									  ...
									  */
									double areaABC = osaCA.x * osaAB.y - osaCA.y * osaAB.x;
									// Barycentrické souřadnice pro interpolaci hloubky
									double u = (osaBC.x * vec.y - osaBC.y * vec.x) / areaABC;
									double v = (osaCA.x * vec2.y - osaCA.y * vec2.x) / areaABC;
									double w = 1.0 - u - v;

									// Jednoduchá lineární interpolace hloubky
									double depth = u * triangles[0].depth + v * triangles[1].depth + w * triangles[2].depth;
