	...
	public static Scene makeScene()
		{
			...
			Scene scene = new Scene( c, 311, 82, Math.PI / 20, 0.1, 1000.1, PrimitiveType.PT_Triangle ); //311 vs 52
			Move m = new Move([0.0, -5.0, -25]);
			Move m2 = new Move([7.0, -5.0, -15]);
			Move mPlane = new Move([ 0.0, -4, -15 ] );
	    	Scale mScale = new Scale ([ 15.0, 15.0, 1.0 ] );
			Rotation q = new Rotation( -Math.PI/2, Osa.x);
			Kuzel kuzel = new Kuzel( '.', ConsoleColor.Red );
			kuzel.GetTriangulated();

			Tree pq = new Tree( 'o', ConsoleColor.Green );
			
			Plane_2d plane = new Plane_2d( '.', ConsoleColor.Green );
			plane.GetTriangulated();
			pq.GetTriangulated();
			Rotation rq = new Rotation ( Math.PI / 2 , Osa.x );
			kuzel.addTransformation(m2);
			pq.addTransformation(rq);
			pq.addTransformation(m);
			plane.addTransformation( mScale );
		    plane.addTransformation(rq);
			plane.addTransformation( mPlane );
